 ...
 * The prompt registry is fetched from sigeconomy.com/prompts.json (static asset,
...
const DEFAULT_SATELLITE_BASE =
  process.env.SIGRANK_SATELLITE_BASE || "https://sigeconomy.com";
  ...
  },
  {
    ...
    outputSchema: {
      ...
      properties: {
        ...
        shareable_url: { type: "string", description: "Link to the full ranking on sigeconomy.com" },
...
export async function callTool(name, args) {
  ...
  if (name === "get_prompt_of_the_day") {
    // Fetch the prompt registry from sigeconomy.com (static asset, served from
    // Cloudflare's ASSETS binding — zero Worker invocations).
    const url = `${DEFAULT_SATELLITE_BASE}/prompts.json`;
    const res = await robustFetch(url, {
      headers: { accept: "application/json", "user-agent": "bestuser-router-mcp/0.1.0" },
    });
    if (!res.ok) throw new Error(`sigeconomy.com/prompts.json → HTTP ${res.status}`);
